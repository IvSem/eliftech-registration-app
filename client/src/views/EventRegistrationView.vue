...
const { data: event, fetchData: fetchEventById } = useAxios(`/api/events/${route.params.eventId}`)
...
<template>
  <main>
    <div>"{{ event?.title }}" registration</div>
    <form @submit.prevent="submitForm" class="w-full md:max-w-[50%]">
      <MyInput
        label="Full name"
        type="text"
        name="fullName"
