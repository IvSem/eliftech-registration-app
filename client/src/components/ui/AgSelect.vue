<script setup>
import { reactive, computed, defineProps, defineEmits, watch, onMounted } from 'vue'
...
const onEscape = (e) => {
  if (
    !(
      e.relatedTarget &&
      (e.relatedTarget.id == state.searchId || e.relatedTarget.id == state.wrapperId)
    )
  ) {
...
<template>
  ...
  >
    <div class="current" @click="onWrapperClick">
      <span :class="{ 'is-selected': !!state.selectedOption }" @click="onWrapperClick">{{
        selectedText
      }}</span
      ><i v-if="state.selectedOption" class="clear" @click.stop="onClear">&times;</i>
    </div>
